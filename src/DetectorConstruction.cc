...
G4VPhysicalVolume* DetectorConstruction::Construct() {
    ...
    for (int i=0; i<N; ++i) {
      for (int j=0; j<N; ++j) {
        ...
        for (auto& gp : guidePositions) {
          if (gp.first==i && gp.second==j) { isGuide = true; break; }
        }
        if (isGuide) {
    ...
    // Görselleştirme
    auto visFuel  = new G4VisAttributes(G4Colour(1,0,0));      visFuel->SetForceSolid(true);  logicFuel->SetVisAttributes(visFuel);
    auto visClad  = new G4VisAttributes(G4Colour(0.8,0.8,0.8)); visClad->SetForceSolid(true);  logicClad->SetVisAttributes(visClad);
    auto visCell  = new G4VisAttributes(G4Colour(0,0,1,0.2));  visCell->SetForceSolid(true);  logicCell->SetVisAttributes(visCell);
    auto visGuide = new G4VisAttributes(G4Colour(0.2,0.2,0.8,0.5)); visGuide->SetForceSolid(true); logicGuide->SetVisAttributes(visGuide);
    auto visInstr = new G4VisAttributes(G4Colour(0.5,0.5,1.0,0.7)); visInstr->SetForceSolid(true); logicInstr->SetVisAttributes(visInstr);
